...
{
	if (Dept[dept_ID]->GetMgr() != NULL) {
		cout << "There is Already a Manager. Remove First\n";
		return;
	}
...
void Main::view_cmp_details(vector<Complaint*> cmps_in_range, int ind)
{
	if (ind >= 0 && ind < cmps_in_range.size()) {
		cout << "\n------------COMPLAINT DETAILS--------------\n";
		cout << "ID: " << cmps_in_range[ind]->getID()
			<<"\nDate :" << cmps_in_range[ind]->GetTime().tm_year << " - " << cmps_in_range[ind]->GetTime().tm_mon << " - " << cmps_in_range[ind]->GetTime().tm_mday
...
vector<Teacher*> Main::GetTeachers()
{
	return teachers;
}
vector<Department*> Main::Get_Depts()
{
	return Dept;
}
Main::~Main()
